...
# Scenario A: A1,A3,N (train) and A1,A3,N (test)
# Scenario B: A1,N (train) and A2,A4,N (test)
# Scenario C: A1,A2,N (train) and A1,A2,A3,N (test)

while True:
    choice = input("Choose a scenario, A B or C: ")
    if choice.upper() == 'A':
        training = 'Training-a1-a3.csv'
        testing = 'Testing-a1-a3.csv'
        break
    elif choice.upper() == 'B':
        training = 'Training-a1.csv'
        testing = 'Testing-a2-a4.csv'
        break
    elif choice.upper() == 'C':
        training = 'Training-a1-a2.csv'
        testing = 'Testing-a1-a2-a3.csv'
        break
...
# TrainingDataPath='/home/ubuntu/lab-cs-ml-00301/NSL-KDD/'
curdir='/home/ubuntu/lab-cs-ml-00301/'
# TrainingData='KDDTrain+_20Percent.txt'
# TrainingData='KDDTrain+.txt' #take out for now
...
# NEW IMPORT DATASET!
# We use data_preprocessor given to us
import data_preprocessor as dp
X_train, y_train = dp.get_processed_data(
    curdir+training, 
    curdir+"mappings/",
    classType='binary'
)
X_test, y_test = dp.get_processed_data(
    curdir+testing, 
    curdir+"mappings/", 
    classType='binary'
)
...
classifierHistory = classifier.fit(X_train, y_train,
                                   batch_size = BatchSize, epochs = NumEpoch)
